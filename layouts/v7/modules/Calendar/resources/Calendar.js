...
class Calendar_Calendar_Js extends Vtiger_Class_Js {
    static calendarViewContainer = false;
    static feedsWidgetPostLoadEvent = 'Calendar.Viewtypes.PostLoad.Event';
    static disabledFeedsStorageKey = 'calendar.feeds.disabled';
    static calendarInstance = false;
    static numberOfDaysInAgendaView = 7;
    static userPreferenceCache = [];
    static sideBarEssentialsState = '';
    ...
    static showCreateEventModalforDrag() {
        var instance = Calendar_Calendar_Js.getInstance();
    ...
    static showCalendarSettings() {
        var instance = Calendar_Calendar_Js.getInstance();
        instance.showCalendarSettings();
    }

    static deleteCalendarEvent(eventId, sourceModule, isRecurring) {
        var instance = Calendar_Calendar_Js.getInstance();
        instance.deleteCalendarEvent(eventId, sourceModule, isRecurring);
    }

    static editCalendarEvent(eventId, isRecurring) {
        var instance = Calendar_Calendar_Js.getInstance();
        instance.editCalendarEvent(eventId, isRecurring);
    }

    static copyCalendarEvent(eventId, isRecurring) {
        var instance = Calendar_Calendar_Js.getInstance();
        instance.copyCalendarEvent(eventId, isRecurring, true);
    }

    static editCalendarTask(taskId) {
        var instance = Calendar_Calendar_Js.getInstance();
        instance.editCalendarTask(taskId);
    }

    static markAsHeld(recordId, sourceModule) {
        var instance = Calendar_Calendar_Js.getInstance();
        instance.markAsHeld(recordId,sourceModule);
    }

    static holdFollowUp(eventId) {
        var instance = Calendar_Calendar_Js.getInstance();
    ...
    registerCreateFollowUpEvent(modalContainer) {
    ...
    updateCalendarView(activitytype) {
    ...
    markAsHeld(recordId, sourceModule) {
        var thisInstance = this;
    ...
    registerCalendarSharingTypeChangeEvent(modalContainer) {
    ...
    registerHourFormatChangeEvent(modalContainer) {
    ...
    registerCalendarSettingsShownEvents(modalContainer) {
    ...
    assignFeedTextColor(feedCheckbox) {
    ...
    restoreFeedsState(widgetContainer) {
        var thisInstance = this;
        var disabledFeeds = this.getDisabledFeeds();
    ...
    updateRangeFields(container, options) {
    ...
    initializeColorPicker(element, customParams, onChangeFunc) {
        var params = {
            flat: true,
    ...
    registerDateFieldChangeEvent(modalContainer) {
        var thisInstance = this;
    ...
    _getParsedConditions(conditions) {
    ...
    saveFeedSettings(modalContainer, feedIndicator) {
        var thisInstance = this;
    ...
    registerColorEditorSaveEvent(modalContainer, feedIndicator) {
        var thisInstance = this;
...
//handling eventtype condition element display
    ...
    registerAddActivityTypeEvent(modalContainer) {
        var thisInstance = this;
        modalContainer.find('[name="saveButton"]').on('click', function () {
            var currentTarget = jQuery(this);
            currentTarget.attr('disabled', 'disabled');
    ...
    registerAddActivityTypeFeedActions(modalContainer) {
        var thisInstance = this;
        var colorPickerHost = modalContainer.find('.calendarColorPicker');
        var selectedColor = modalContainer.find('.selectedColor');
        thisInstance.initializeColorPicker(colorPickerHost, {}, function (hsb, hex, rgb) {
...
//handling eventtype condition element display
    ...
    registerFeedAddEvent(widgetContainer) {
        var thisInstance = this;
...
//thisInstance.restoreWidgetState(widget);
    ...
    transformToEventObject(eventData, feedCheckbox) {
        var eventObject = {};
...
//since support for multiple calendar views for events is enabled
    ...
    validateAndSaveEvent(modalContainer) {
        var thisInstance = this;
        var params = {
            submitHandler: function (form) {
    ...
    registerCreateEventModalEvents(modalContainer) {
        this.validateAndSaveEvent(modalContainer);
    }

    setStartDateTime(modalContainer, startDateTime) {
    ...
    setEndDateTime(modalContainer, endDateTime) {
    ...
    showCreateModal(moduleName, startDateTime) {
    ...
    showCreateModalforDrag(moduleName, startDateTime, endDateTime) {
        var isAllowed = jQuery('#is_record_creation_allowed').val();
        if (isAllowed) {
            var thisInstance = this;
            var quickCreateNode = jQuery('#quickCreateModules').find('[data-name="' + moduleName + '"]');
    ...
    showCreateEventModal(startDateTime) {
        this.showCreateModal('Events', startDateTime);
    }

    showCreateTaskModalforDrag() {
        this.showCreateModalforDrag('Calendar');
    }

    showCreateEventModalforDrag(startDateTime, endDateTime) {
        this.showCreateModalforDrag('Events', startDateTime, endDateTime);
    }

    updateAllTasksOnCalendar() {
        this._updateAllOnCalendar("Calendar");
    }

    showTaskOnCalendar(data) {
        this.updateAllTasksOnCalendar();
    }

    updateCalendar(calendarModule, data) {
    ...
    registerPostQuickCreateSaveEvent() {
        var thisInstance = this;
    ...
    performDayClickAction(date, jsEvent, view) {
    ...
    performDayDragAction(startDate, endDate, jsEvent, view) {
        if (!this.performingDayClickOperation) {
            this.performingDayClickOperation = true;
            // if (date.hasTime() || view.type == 'month') {
    ...
    _updateEventOnResize(postData, revertFunc) {
        var thisInstance = this;
        app.helper.showProgress();
    ...
    updateEventOnResize(event, delta, revertFunc, jsEvent, ui, view) {
        var thisInstance = this;
    ...
    updateEventOnDrop(event, delta, revertFunc, jsEvent, ui, view) {
        var thisInstance = this;
        if (event.module !== 'Calendar' && event.module !== 'Events') {
            revertFunc();
            return;
    ...
    getActivityTypeClassName(activitytype) {
    ...
    addActivityTypeIcons(event, element) {
    ...
    _deleteCalendarEvent(eventId, sourceModule, extraParams) {
        var thisInstance = this;
    ...
    deleteCalendarEvent(eventId, sourceModule, isRecurring) {
        var thisInstance = this;
        if (isRecurring) {
    ...
    updateEventOnCalendar(eventData) {
        this.updateAllEventsOnCalendar();
    }

    _updateEvent(form, extraParams) {
    ...
    validateAndUpdateEvent(modalContainer, isRecurring) {
        var thisInstance = this;
        var params = {
            submitHandler: function (form) {
                if (this.numberOfInvalids() > 0) {
    ...
    registerEditEventModalEvents(modalContainer, isRecurring) {
        this.validateAndUpdateEvent(modalContainer, isRecurring);
    }

    showEditModal(moduleName, record, isRecurring, isDuplicate) {
        var thisInstance = this;
        var quickCreateNode = jQuery('#quickCreateModules').find('[data-name="' + moduleName + '"]');
        if (quickCreateNode.length <= 0) {
            app.helper.showAlertNotification({
    ...
    showEditEventModal(eventId, isRecurring, isDuplicate) {
        this.showEditModal('Events', eventId, isRecurring, isDuplicate);
    }

    editCalendarEvent(eventId, isRecurring) {
        this.showEditEventModal(eventId, isRecurring);
    }

    copyCalendarEvent(eventId, isRecurring, isDuplicate) {
        this.showEditEventModal(eventId, isRecurring, isDuplicate);
    }

    registerPopoverEvent(event, element, calendarView) {
    ...
    performPreEventRenderActions(event, element) {
        var calendarView = this.getCalendarViewContainer().fullCalendar('getView');
        this.addActivityTypeIcons(event, element);
        this.registerPopoverEvent(event, element, calendarView);
    }

    performMouseOutActions(event, jsEvent, view) {
//var currentTarget = jQuery(jsEvent.currentTarget);
    }

    performMouseOverActions(event, jsEvent, view) {
//var currentTarget = jQuery(jsEvent.currentTarget);
    }

    getCalendarHeight(view) {
        var portion = 0.86;
...
//cleanup before render
    ...
    registerGotoDateButtonAction(navigationsContainer) {
        var thisInstance = this;
    ...
    performPostRenderCustomizations() {
    ...
    performSidebarEssentialsRecognition() {
