...
class Calendar_Edit_Js extends Vtiger_Edit_Js {
    static uploadAndParse() {
    ...
    static userChangedTimeDiff = false;
    relatedContactElement = false;
    recurringEditConfirmation = false;

    getRelatedContactElement(form) {
    ...
    registerRelatedContactSpecificEvents(form) {
        var thisInstance = this;
        if(typeof form == "undefined") {
            form = this.getForm();
        }
    ...
    getPopUpParams(container) {
        var params = super.getPopUpParams(container);
    ...
    resetRecurringDetailsIfDisabled(form) {
    ...
    addNewContactToRelatedList(newContactInfo, form) {
        if(form.length <= 0) {
            form = this.getForm();
        }
    ...
    referenceCreateHandler(container) {
        ...
        if(container.find('.sourceField').attr('name') != 'contact_id'){ 
            super.referenceCreateHandler(container); 
            return; 
        }
    ...
    */
    ...
    registerDateStartChangeEvent(container) {
    ...
    registerTimeEndChangeEvent(container) {
    ...
    registerDateEndChangeEvent(container) {
        container.find('[name="due_date"]').on('change', function() {});
    }

    registerActivityTypeChangeEvent(container) {
    ...
    registerUserChangedDateTimeDetection(container) {
        var initialValue;
        container.on('focus',
    ...
    registerDateTimeHandlersEditView(container) {
       var thisInstance = this;
    ...
    registerDateTimeHandlers(container) {
        var thisInstance = this;
    ...
    registerToggleReminderEvent(container) {
     ...
     */
    registerRecurrenceFieldCheckBox(container) {
