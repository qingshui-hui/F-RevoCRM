...
class MailManager_List_Js extends Vtiger_List_Js {
    getContainer() {
    ...
    registerDeleteMailboxEvent(data) {
        var settingContainer = jQuery(data);
    ...
    openFolder(folderName, page, query, type) {
        var self = this;
        app.helper.showProgress(app.vtranslate("JSLBL_Loading_Please_Wait")+"...");
        if(!page) {
            page = 0;
    ...
    updatePagingCount(deletedCount) {
    ...
    registerMailCheckBoxClickEvent() {
        var self = this;
        var container = self.getContainer();
    ...
    registerMoveMailDropdownClickEvent() {
        var self = this;
        var container = self.getContainer();
    ...
    registerMainCheckboxClickEvent() {
        var self = this;
        var container = self.getContainer();
    ...
    openDraftFolder(page, query, type) {
        var self = this;
        app.helper.showProgress(app.vtranslate("JSLBL_Loading_Please_Wait")+"...");
        if(!page) {
            page = 0;
    ...
    registerDraftPrevPageClickEvent() {
        var self = this;
        var container = self.getContainer();
        container.find('#PreviousPageButton').click(function(e) {
            var element = jQuery(e.currentTarget);
    ...
    registerDraftNextPageClickEvent() {
        var self = this;
        var container = self.getContainer();
        container.find('#NextPageButton').click(function(e) {
            var element = jQuery(e.currentTarget);
    ...
    updateUnreadCount(count, folder) {
        var self = this;
        var container = self.getContainer();
        if(!folder) {
    ...
    associateComment(relatedRecord) {
        var self = this;
        var container = self.getContainer();
        var params = {
            'module' : 'MailManager',
    ...
    resetRelationDropdown() {
        this.getContainer().find('#_mlinktotype').val("");
    }

    openComposeEmailForm(type, params, data) {
