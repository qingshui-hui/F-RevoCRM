...
class Inventory_Edit_Js extends Vtiger_Edit_Js {
    static zeroDiscountType = 'zero';
    static percentageDiscountType = 'percentage';
    static directAmountDiscountType = 'amount';
    static individualTaxType = 'individual';
    static groupTaxType = 'group';

    static lineItemPopOverTemplate = '<div class="popover lineItemPopover" role="tooltip"><div class="arrow"></div>\n\
     ...
     */
    copyAddressDetails(data, container, addressMap) {
        var self = this;
     ...
     */
    mapAddressDetails(addressDetails, result, container) {
     ...
     * @return : string
     */
    getPurchaseCostValue(lineItemRow) {
        var rowNum = this.getLineItemRowNumber(lineItemRow);
     ...
     */
    setPurchaseCostValue(lineItemRow, purchaseCost) {
     ...
     * @return : current instance;
     */
    setImageTag(lineItemRow, imgSrc) {
     ...
     * @return : string
     */
    getListPriceValue(lineItemRow) {
        return parseFloat(lineItemRow.find('.listPrice').val());
    }

    setListPriceValue(lineItemRow, listPriceValue) {
        var listPrice = parseFloat(listPriceValue).toFixed(this.numOfCurrencyDecimals);
        lineItemRow.find('.listPrice').val(listPrice);
        return this;
    }
     ...
     * @return : current instance;
     */
    setLineItemTotal(lineItemRow, lineItemTotalValue) {
     ...
     */
    getLineItemTotalElement(lineItemRow) {
     ...
     * @return : current instance;
     */
    setDiscountTotal(lineItemRow, discountValue) {
     ...
     * @return : current instance;
     */
    setTotalAfterDiscount(lineItemRow, totalAfterDiscountValue) {
     ...
     * @return : string
     */
    getTotalAfterDiscount(lineItemRow) {
     ...
     * @return : current instance;
     */
    setLineItemTaxTotal(lineItemRow, taxTotal) {
     ...
     * @return : string
     */
    getLineItemTaxTotal(lineItemRow) {
     ...
     * @return : current instance;
     */
    setLineItemNetPrice(lineItemRow, lineItemNetPriceValue) {
     ...
     * @return : string
     */
    getLineItemNetPrice(lineItemRow) {
        return this.formatLineItemNetPrice(lineItemRow.find('.netPrice'));
    }

    formatLineItemNetPrice(netPriceEle) {
     ...
     */
    setFinalDiscountTotal(finalDiscountValue) {
        this.finalDiscountTotalEle.text(finalDiscountValue);
        return this;
    }

    getFinalDiscountTotal() {
    ...
    setGroupTaxTotal(groupTaxTotalValue) {
     ...
     */
    setPreTaxTotal(preTaxTotalValue) {
     ...
     * @return : current instance;
     */
    setMarginValue(lineItemRow, margin) {
        var rowNum = this.getLineItemRowNumber(lineItemRow);
     ...
     * @return : jQuery object
     */
    getClosestLineItemRow(element) {
        return element.closest('tr.'+this.lineItemDetectingClass);
    }

    isProductSelected(element) {
    ...
    clearLineItemDetails(parentElem) {
     ...
     */
    savePreTaxTotalValue() {
        jQuery('#pre_tax_total').val(this.getPreTaxTotal());
    }

    updateRowNumberForRow(lineItemRow, expectedSequenceNumber, currentSequenceNumber) {
     ...
     */
    initializeLineItemRowCustomFields(lineItemRow, rowNum) {
    ...
    getLineItemSetype(row) {
        return row.find('.lineItemType').val();
    }

    getNewLineItem(params) {
     ...
     */
    calculateLineItemTotal(lineItemRow) {
        var quantity = this.getQuantityValue(lineItemRow);
     ...
     * @params : lineItemRow - element which will represent lineItemRow
     */
    calculateDiscountForLineItem(lineItemRow) {
        var discountContianer = lineItemRow.find('div.discountUI');
        var element = discountContianer.find('input.discounts').filter(':checked');
     ...
     */
    calculateTaxForLineItem(lineItemRow) {
        var self = this;
     ...
     */
    calculateLineItemNetPrice(lineItemRow) {
        var totalAfterDiscount = this.getTotalAfterDiscount(lineItemRow);
...
//		this.calculateGrandTotal();
    ...
    lineItemDirectDiscountCal(conversionRate) {
    ...
    AdjustmentShippingResultCalculation(conversionRate) {
        //Adjustment
        var self = this;
    ...
    lineItemRowCalculations(lineItemRow) {
    ...
    lineItemToTalResultCalculations() {
    ...
    taxPercentageChangeActions(lineItemRow) {
        this.calculateLineItemNetPrice(lineItemRow);
        this.calculateNetTotal();
        this.calculateFinalDiscount();
        if(this.isGroupTaxMode()){
    ...
    lineItemDiscountChangeActions(lineItemRow) {
        this.calculateDiscountForLineItem(lineItemRow);
        this.calculateTaxForLineItem(lineItemRow);
        this.calculateLineItemNetPrice(lineItemRow);
    ...
    lineItemDeleteActions() {
        this.lineItemToTalResultCalculations();
    }

    loadSubProducts(lineItemRow) {
     ...
     * @params : lineItemRow - element which will represent lineItemRow
     */
    quantityChangeActions(lineItemRow) {
    ...
    mapResultsToFields(parentRow, responseData) {
    ...
    showLineItemPopup(callerParams) {
        var params = {
    ...
    postLineItemSelectionActions(itemRow, selectedLineItemsData, lineItemSelectedModuleName) {
     ...
     */
    pricebooksPopupHandler(popupImageElement) {
        var self = this;
    ...
    registerProductAndServiceSelector() {
        var self = this;
    ...
    registerReducedtaxrateChangeEvent() {
       var self = this;
    ...
    */
    ...
    registerLineItemDiscountShowEvent() {
       var self = this;
...
//						}
    ...
    registerFinalDiscountShowEvent() {
        var self = this;
    ...
    registerFinalDiscountChangeEvent() {
        var self = this;
    ...
    registerChargeBlockChangeEvent() {
        var self = this;
        var chargesBlockContainer = this.chargesContainer;
    ...
    registerChargeTaxesChangeEvent() {
        var self = this;
    ...
    registerDeductTaxesChangeEvent() {
        var self = this;
    ...
    registerClearLineItemSelection() {
        var self = this;
...
//			});
     ...
     */
    registerForTogglingBillingandShippingAddress() {
    ...
    registerLineItemAutoComplete(container) {
        var self = this;
...
//			};
     ...
     */
    registerReferenceSelectionEvent(container) {
        var self = this;
     ...
     */
    referenceSelectionEventHandler(data, container) {
        var self = this;
    ...
    registerBasicEvents(container) {
        super.registerBasicEvents(container);
