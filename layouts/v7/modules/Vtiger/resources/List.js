...
class Vtiger_List_Js extends Vtiger_Class_Js {
    static listInstance = false;
    static filterClick = false;
    ...
    static triggerSendSms(massActionUrl, module) {
    ...
    static triggerExportAction(exportActionUrl) {
        var listInstance = window.app.controller();
        listInstance.performExportAction(exportActionUrl);
    }

    static triggerPDFExportAction(exportActionUrl) {
        var listInstance = window.app.controller();
     ...
     */
    static triggerSendEmail(massActionUrl, module, params) {
        var listInstance = window.app.controller();
    ...
    static triggerTransferOwnership(massActionUrl) {
        var listInstance = window.app.controller();
    ...
    static triggerMassAction(massActionUrl) {
    ...
    static showDuplicateSearchForm(url) {
        app.helper.showProgress();
    ...
    static triggerMergeRecord() {
        var listInstance = window.app.controller();
        listInstance.performMergeRecords();
    }

    static triggerAddStar() {
        var listInstance = app.controller();
        var listSelectParams = listInstance.getListSelectAllParams(true)
        if (listSelectParams) {
    ...
    static triggerPreviewForRecord(recordId, appName) {
        var listInstance = Vtiger_List_Js.getInstance();
        listInstance.showQuickPreviewForId(recordId, appName, '');
        return false;
    }

    static triggerAddTag() {
        var listInstance = app.controller();
        var listInstance = app.controller();
        var listSelectParams = listInstance.getListSelectAllParams(true);
        if (listSelectParams) {
     ...
     * @returns {undefined}
     */
    static previewFile(e, recordId, attachmentId) {
        e.stopPropagation();
        if (recordId) {
            var params = {
                module: 'ModComments',
    ...
    setListViewContainer(container) {
    ...
    getRecordSelectTrackerInstance() {
...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
    ...
    addStarSearchParams(listSearchParams) {
        var listViewContainer = this.getListViewContainer();
     ...
     * @returns {undefined}
     */
    registerInlineEdit(currentTrElement) {
        var self = this;
    ...
    registerEventForPicklistDependencySetup(container) {
    ...
    validateAndSaveInlineEdit(currentTrElement) {
        var listViewContainer = this.getListViewContainer();
        var thisInstance = this;
        var tdElements = jQuery('.listViewEntryValue', currentTrElement);
...
//			tableContainer.perfectScrollbar('update');
     ...
     * @return value
     */
    getInlineEditedFieldValue(fieldElement, inputElement) {
        var value = null;
     ...
     */
    getReferenceSearchParams(element) {
     ...
     */
    registerFieldActionEvents(editedRow) {
        var vtigerInstance = Vtiger_Index_Js.getInstance();
    ...
    registerInlineSaveOnEnterEvent(editedRow) {
...
//			tableContainer.perfectScrollbar('update');
    ...
    saveMassEdit(event, form_original_data, isOwnerChanged) {
    ...
    getListSelectAllParams(jsonDecode) {
            var self = this;
    ...
    selectRecordsDependingOnStarFilter() {
        if (this.isStarFilterMode()) {
...
//			e.stopPropagation();
    ...
    _deleteRecord(recordId, extraParams) {
        var thisInstance = this;
        var module = app.getModuleName();
        var postData = {
            "data": {
    ...
    deleteRecord(recordId, extraParams) {
        var thisInstance = this;
    ...
    registerEventToShowQuickPreview() {
        var self = this;
        var listViewPageDiv = self.getListViewContainer();
    ...
    registerMoreRecentUpdatesClickEvent(container, recordId) {
    ...
    registerNextRecordClickEvent(container) {
        var self = this;
    ...
    registerPreviousRecordClickEvent(container) {
        var self = this;
    ...
    checkAndAddTagsToList(tagEleList) {
        var self = this;
        var container = jQuery('#listViewTagContainer');
    ...
    addToExistingTagSelector(tagEle) {
    ...
    registerDynamicDropdownPosition(container, relativeto) {
    ...
    registerPostLoadListViewActions() {
        var self = this;
        var recordSelectTrackerObj = self.getRecordSelectTrackerInstance();
    ...
    registerConfigureColumnsEvents() {
        var thisInstance = this;
        var listViewContentDiv = this.getListViewContainer();
     ...
     * @returns {unresolved}
     */
    getFieldValue(fieldName, sourceElement) {
     ...
     * @returns {undefined}
     */
    registerEventToHandleStickyHeadOnWindowScroll() {
        var thisInstance = this;
...
//      
    ...
    registerHeaderReflowOnListSearchSelections() {
        var listViewContentDiv = this.getListViewContainer();
        var thisInstance = this;
        var $table = jQuery('#listview-table');
    ...
    registerDropdownPosition(container) {
