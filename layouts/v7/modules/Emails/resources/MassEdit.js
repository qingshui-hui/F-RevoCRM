...
class Emails_MassEdit_Js {
     ...
     */
    registerEmailFieldSelectionEvent(callBack) {
        var thisInstance = this;
    ...
    registerPreventFormSubmitEvent() {
     ...
     * @returns {undefined} 
     */
    registerSelectEmailTemplateEvent() {
        var thisInstance = this;
    ...
    registerSaveDraftOrSendEmailEvent() {
        var thisInstance = this;
        var form = this.getMassEmailForm();
    ...
    removeFromEmailAddressData(mailInfo) {
    ...
    removeFromSelectedIds(selectedId) {
    ...
    addToEmailAddressData(mailInfo) {
        var mailInfoElement = this.getMassEmailForm().find('[name="toemailinfo"]');
    ...
    appendToSelectedIds(selectedId) {
        var selectedIdElement = this.getMassEmailForm().find('[name="selected_ids"]');
        var previousValue = '';
    ...
    removeAttachmentFileSizeByElement(element) {
        this.attachmentsFileSize -= element.get(0).files[0].size;
    }

    removeDocumentsFileSize(documentSize) {
        this.documentsFileSize -= parseFloat(documentSize);
    }

    removeAttachmentFileSizeBySize(fileSize) {
        this.attachmentsFileSize -= parseFloat(fileSize);
    }
    ...
    setReferenceFieldValue(container, object) {
       var thisInstance = this;
       var preloadData = thisInstance.getPreloadData();
    ...
    fileAfterSelectHandler(element, value, master_element) {
        var thisInstance = this;
    ...
    getDocumentAttachmentElement(selectedFileName, id, selectedFileSize) {
     ...
     */
    checkIfExisitingAttachment(selectedDocumentId) {
        var documentExist;
        var documentPresent;
        var mode = jQuery('[name="emailMode"]').val();
    ...
    writeDocumentIds(selectedDocumentId) {
        var thisInstance = this;
        var newAttachment;
        var selectedDocumentIds = jQuery('#documentIds').val();
        if(selectedDocumentIds != ""){
    ...
    removeDocumentIds(removedDocumentId) {
    ...
    registerEventForRemoveCustomAttachments() {
        var thisInstance = this;
        var composeEmailForm = this.getMassEmailForm();
